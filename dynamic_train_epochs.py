...
from dynamic_trainer_epochs import DynamicTrainerEpochs
...
if __name__ == '__main__':
    ...
    epsilon_k_list = [(16, 512), (32, 256), (64, 128)]

    trainer = DynamicTrainerEpochs(model=model, optimizer=optimizer,
                      cfg=cfg, epsilon_k_list=epsilon_k_list,
    ...
    for epoch in range(cfg.max_epoch):
        ...
        subset = RandomSampler(train_set, num_samples=int((0.2*len(train_set))))
        subset_loader = DataLoader(train_set, batch_size=cfg.batch_size, sampler=subset)
        print(type(subset_loader))

        trainer.train(epoch, train_loader, subset_loader)
