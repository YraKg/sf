...
def wideresnet70_16(**kwargs):
    return WideResNet(depth=40,widen_factor=16, **kwargs)
def FT_wideresnet70_16(**kwrags):
    model = load_model(model_name='Bartoldson2024Adversarial_WRN-82-8', dataset='cifar10', threat_model='Linf')
    return model

def FT_wrn_34_20(**kwrags):
    model = load_model(model_name='Chen2024Data_WRN_34_20', dataset='cifar10', threat_model='Linf')
    return model

def FT_wrn_94_16(**kwrags):
    model = load_model(model_name='Bartoldson2024Adversarial_WRN-94-16', dataset='cifar10', threat_model='Linf')
    return model

def FT_preactresnet(**kwrags):
    model = load_model(model_name='Gowal2021Improving_R18_ddpm_100m', dataset='cifar10', threat_model='Linf')
    return model
def FT_Swin_L(**kawrgs):
    model = load_model(model_name='Liu2023Comprehensive_Swin-L', dataset='imagenet', threat_model='Linf')
    return MappedImagenetModel(model)

def FT_wrn_50_2_imagenet(**kawrgs):
    model = load_model(model_name='Chen2024Data_WRN_50_2', dataset='imagenet', threat_model='Linf')
    return MappedImagenetModel(model)

def FT_Swin_L_mixed(**kawrgs):
    model = load_model(model_name='Bai2024MixedNUTS', dataset='imagenet', threat_model='Linf')
    return MappedImagenetModel(model)
...
__factory = {
    ...
    'wideresnet70_16': wideresnet70_16, 
    ...
    'lenet': LeNet,
    #Fine tuning models from robust bench
    #CIFAR-10
    "FT_wrn70_16":FT_wideresnet70_16,
    "FT_wrn34_20": FT_wrn_34_20,
    "FT_wrn94_16": FT_wrn_94_16,
    #Imagenet
    "FT_swin-L": FT_Swin_L,
    "FT_swin-L_mixed": FT_Swin_L_mixed,
    "FT_wrn50_2_imagenet":FT_wrn_50_2_imagenet
}
import json
class MappedImagenetModel(nn.Module):
    def __init__(self,model):
        super().__init__()
        self.model = model
        with open('./data/imagenet100/Real_Labels.json', 'r') as file:
            self.relevant_indexes = json.load(file)

        
    def forward(self,x):
        logits = self.model(x)
        mapped = logits[:,self.relevant_indexes]
        return mapped
